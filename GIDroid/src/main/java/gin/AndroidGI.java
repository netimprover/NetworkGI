...
import gin.util.ConditionCount;
...
//        spea2.run();
    ...
    public void localSearch() {
        ...
        AndroidUnitTestResultSet origRes = testPatchLocally(patch, true, 1);
        ...
        for (int step = 0; step < 400; step++) {
            ...
            try {
                ...
                if (!tested.containsKey(neighbour.toString())) {
                    results = testPatchLocally(neighbour, true, 1);
    ...
    public void GP() {
        ...
        for (int g = 0; g < genNumber; g++) {
            ...
            for (Patch patch : crossoverPatches) {
                ...
                if (!tested.containsKey(patch.toString())) {
                    results = testPatchLocally(patch, true, 1);

                } else {
                    results = tested.get(patch.toString());
                }
                population.put(patch, (double) results.getNetworkUsage());
